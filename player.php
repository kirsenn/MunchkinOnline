...
if ( (isset($_SESSION['id_user'])) && (isset($_SESSION['init'])) && (isset($_SESSION['id_gt'])) )
{
    ...
    if ($_REQUEST['send_com']==0) 
	{
		...
		{//Информация по собственному игрок 
			...
			$showelement=$showelement.'<b>Имя: </b>'.$row['login'].'('.$row['level'].')<br/>';
			$showelement=$showelement.'<div id="sex1"><b>Пол: </b>'.$row['sex'].'</div>';
			$showelement=$showelement.'</div>';
		  
			if ( ($row['sex']=="м") || ($row['sex']=="m") ){
			  $showelement=$showelement.'<div id="change_female">Сменить пол на Ж</div>';
			}else{
			  $showelement=$showelement.'<div id="change_male">Сменить пол на М</div>';			  
			}
		  
			//Отображаем сколько карт у пользователя на руках
			$query1="SELECT * FROM cards_of_user JOIN cards ON (cards_of_user.id_card=cards.id_card) WHERE (id_user=".$_SESSION['id_user'].")";
			$result1=mysql_query($query1);
		  
			echo show_back_card($result1,$showelement); 
		}else
		{//Информация по сопернику
			$query="SELECT * FROM users 
                    WHERE (id_user=".$_REQUEST['id_user'].")";       
			$result=mysql_query($query);
			$row=mysql_fetch_array($result);
    ...
    }elseif ($_REQUEST['send_com']==1) 
	{
        ...
        if ($_REQUEST['id_user']==$_SESSION['id_user'])
		{
            if ($_SESSION['sex']=="woman")
			{  
                $query='UPDATE users SET sex="м" 
                        WHERE (id_user='.$_SESSION['id_user'].')';
                $result=mysql_query($query);  
              
                $per_str=' изменил пол - стал  [B]мужчиной[/B]';
                add_str($per_str,0);     
    ...
    }elseif ($_REQUEST['send_com']==2) 
	{
		...
		if ($_REQUEST['id_user']==$_SESSION['id_user'])
		{
			if ($_SESSION['sex']=="man")
			{   
				$query='UPDATE users SET sex="ж" 
						WHERE (id_user='.$_SESSION['id_user'].')';
				$result=mysql_query($query);
					 
				$per_str='изменил пол - стал  [B]женщиной[/B]';
				add_str($per_str,0);               
				$_SESSION['sex']="woman";
			} 
