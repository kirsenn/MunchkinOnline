              ...
              if (mysql_num_rows($result)!=0){          
                  while ($row=mysql_fetch_array($result)){
                      if ($row['place_card']==30){
                          $per1='<img id="id_card30" class="id_card_class" src="./picture/'.$row['pic'].'" value="30">';
                      }elseif ($row['place_card']==31){
                          $per2='<img id="id_card31" class="id_card_class" src="./picture/'.$row['pic'].'" value="31">';
                      }elseif ($row['place_card']==32){
                          $per3='<img id="id_card32" class="id_card_class" src="./picture/'.$row['pic'].'" value="32">';
                      }        
                  }
                  if (isset($per1)){
                      print '<div id="id_table30" class="u_class_place">'.$per1.'</div>';
                  }else{
                      print '<div id="id_table30"><center>Суперманчкин</center></div>';
                  }  
                  if (isset($per2)){
                      print '<div id="id_table31" class="u_class_place">'.$per2.'</div>' ; 
                  }else{
                      print '<div id="id_table31"><center>Класс 1</center></div>' ; 
                  }               
                  if (isset($per3)){
                      print '<div id="id_table32" class="u_class_place">'.$per3.'</div>' ; 
                  }else{
